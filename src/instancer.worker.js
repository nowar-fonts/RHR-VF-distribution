...
import { Ot, FontIo, Rectify } from 'ot-builder';
...
function generateRhrInstance(font, weight, roundness) {
	self.postMessage({
		status: 'instance-font',
	});
	instanceFont(font, [['wght', normaliseWeight(weight)], ['ROND', normaliseRoundness(roundness)]]);
	self.postMessage({
		status: 'generate-metadata',
	});
	convertToCff1(font);
	buildInstanceMetaData(font, { weight, roundness });
}

function saveToDataUrl(font) {
	self.postMessage({
		status: 'save-otf',
	});
...
function buildFont(weight, roundness) {
	...
	generateRhrInstance(font, weight, roundness);
	const url = saveToDataUrl(font);
	self.postMessage({
		status: 'done',
		url: url,
	});
...
self.onmessage = (ev) => {
	const { weight, roundness } = ev.data;
	const url = buildFont(weight, roundness);
}
